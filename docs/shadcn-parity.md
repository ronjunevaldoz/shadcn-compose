...
| **Item** | `variant: default/outline/muted` × `size: default/sm`; item itself is focusable/interactive (`focus-visible:ring-[3px]`, nested link gets `hover:bg-accent/50`); `ItemMedia` has 3rd `variant: image` (`size-10 rounded-sm`, object-cover) beyond `default`/`icon`; ships `ItemHeader`/`ItemFooter` slots | `ShadcnItemVariant.Default/Outline/Muted` — names match; `ShadcnItemMediaVariant.Default/Icon` — no `Image` case | **Updated 2026-08-04**: ✅ hover + focus-visible ring added, opt-in via a new `onClick: (() -> Unit)? = null` param — real shadcn is only interactive when `asChild`'d to a link/button, and this library has no `asChild`/Slot equivalent, so interactivity is opt-in per-instance instead; `null` (default) keeps every existing call site's prior static-row behavior unchanged. Uses the same shared `focusRing()` extension every other focusable component uses, plus `hovered { background(colors.secondary.copy(alpha = 0.5f)) }` (`secondary` standing in for real's `accent`, same convention as `ToggleStyles.kt`). Fixing this also surfaced and fixed a real bug: the row's own outer `.clip()` was clipping the new focus ring's `dropShadow` | no `size` variant at all — always uses the `default`-equivalent spacing | Missing: `size` variant, `ItemMedia.Image`, `ItemHeader`/`ItemFooter` slots. Note: this repo's JVM/Desktop Roborazzi harness doesn't reliably produce a pixel diff for `focused { dropShadow(...) }` rings under `requestFocus()` (confirmed against `ButtonGroupScreenshotTest`'s existing focused golden too, byte-identical to unfocused) — a pre-existing, library-wide harness limitation, not something this fix introduced |
...
| **HoverCard** | Real Radix defaults: **`openDelay=700ms`, `closeDelay=300ms`** — only opens after ~0.7s continuous hover, lingers ~0.3s after pointer-out; `align="center"`, `sideOffset={4}`, fixed `w-64`(256px) | **Updated 2026-08-04**: `collectIsHoveredAsState()` now drives a `LaunchedEffect(isHovered) { delay(...) }` gate matching real's 700ms open / 300ms close exactly | hover only, 700ms open / 300ms close delay | ✅ width (256dp) and padding both match real exactly | ✅ open/close delay now matches real. Still missing: independent `align` axis, and hovering the popped-open content itself doesn't extend/reset the close timer the way real's content-is-also-hoverable behavior does (not covered by this fix, a separate finer-grained gap) |
| **DropdownMenu** | 15 exports: `Trigger/Content/Group/Item(inset,variant=default\|destructive)/CheckboxItem/RadioGroup/RadioItem/Label(inset)/Separator/Shortcut/Sub/SubTrigger/SubContent`; full keyboard nav (arrow/Home/End/typeahead) from Radix | `ShadcnDropdownMenuItem(label,onClick,enabled,destructive,inset,shortcut)`, `CheckboxItem`, `RadioGroup`/`RadioItem`, `Label(inset)`, `Separator` — 7 pieces; `clickable(indication=null)` means **no hover/press visual**; **no keyboard nav** | enabled/disabled + destructive text color only | ❌ fixed `.width(224.dp)` always; real is `min-w-[8rem]`(128px) and grows/shrinks with content | **Updated 2026-08-04**: `CheckboxItem`, `RadioGroup`/`RadioItem`, `Shortcut`, `inset` landed. Still missing: `Sub`/`SubTrigger`/`SubContent` (**no submenu support at all**) and `Group`; zero focus ring unlike every other focusable component in the library |
| **ContextMenu** | Same 15-export shape as DropdownMenu on `ContextMenuPrimitive`; opens via right-click **or** long-press (touch) **or** the OS context-menu key | `ShadcnContextMenu` detects right-click via `pointerInput`+`isSecondaryPressed`, reuses `ShadcnDropdownMenuScope` (inherits the same CheckboxItem/RadioItem/Shortcut/inset support, and the same missing Sub) | same as DropdownMenu: text-color only, no hover/press/focus, no keyboard nav | fixed `.width(224.dp)`, same grow-to-fit gap as DropdownMenu | No touch long-press, no keyboard-triggered open; Sub/submenu still missing (shared root cause with DropdownMenu) |
...
| **Menubar** | 16 exports on `MenubarPrimitive`; Radix roving-focus means **hovering** an adjacent trigger switches the open menu, no extra click needed | `ShadcnMenubarMenu`/`ShadcnMenubar` — reuses DropdownMenu's items (same CheckboxItem/RadioItem/Shortcut support now, still missing Sub); toggle-on-click only, **no hover-to-switch between open menus**, no keyboard nav | open-highlight per trigger, click-toggle only | real bar fixed `h-9`(36px); ours has no fixed height | Missing the hover-switches-open-menu interaction that makes a real menubar feel native, plus the same Sub/submenu gap as DropdownMenu |
...
| **Dialog** | `Trigger/Portal/Close/Overlay/Content(showCloseButton)/Header/Footer(showCloseButton)/Title/Description`; overlay+content both animate open/close (`fade-in-0`/`zoom-in-95`, `duration-200`) | `ShadcnDialog(visible, onDismissRequest, showCloseButton, dismissOnClickOutside, closeIcon)` — library-wide `visible: Boolean` pattern instead of Trigger/Portal composables (not itself a gap) | open/close via `visible`; close button has a focus ring | ❌ fixed `.width(400.dp)` always; real is responsive, `max-w-lg`(512px) cap that shrinks on narrow viewports | **Updated 2026-08-04**: ✅ entrance now animates (`fadeIn` + `scaleIn(0.95f)`, 200ms — matches real's `fade-in-0`/`zoom-in-95`/`duration-200`). ❌ still no exit animation, same architectural limit as Sheet/Drawer (`ShadcnModalOverlay` tears the Popup down instantly on `visible = false`) — no longer the outlier within the library, now consistent with them |
| **AlertDialog** | `size: default/sm` (`max-w-lg`/`max-w-xs`); new `AlertDialogMedia` icon slot (`size-16 rounded-md bg-muted`); `Action`/`Cancel` are real `Button` wrappers via `asChild` | Thin wrapper over `ShadcnDialog` (`showCloseButton=false, dismissOnClickOutside=false`) — doc comment explicitly rationalizes skipping dedicated `Action`/`Cancel` composables since real's are just styled Buttons | inherits Dialog's states — including its entrance animation, now fixed | same fixed 400dp as Dialog, no size variant | Missing `size` prop and `AlertDialogMedia` slot (both added to the real registry since an earlier pass) — genuinely new divergence, not stale. `Action`/`Cancel`-as-Button omission is a defensible, already-documented design call |
| **Sheet** | `Trigger/Close/Portal/Overlay/Content(side: top/right/bottom/left, showCloseButton)/Header/Footer/Title/Description`; close X shown **by default**; both entry+exit animate (slide-in/slide-out per side); left/right `w-3/4 sm:max-w-sm`(75%/384px cap) | `ShadcnSheet(visible, onDismissRequest, side: Top/Bottom/Start/End, showCloseButton, closeIcon)` — reuses Dialog's Header/Title/Description/Footer **and, as of 2026-08-04, its `DialogCloseButton`** | ✅ entrance slide-in matches real's direction; ❌ **no exit animation** (documented: `ShadcnModalOverlay` tears the Popup down instantly, no window to animate an exit in); ✅ close (X) button now shown by default, matching real | ❌ Start/End fixed `.width(320.dp)` regardless of screen — real's 75%-viewport/384px-cap rule isn't matched either way | Close button fixed 2026-08-04. Remaining gap: no exit animation |
...
| **Combobox** | Real `combobox.tsx` is a first-class Base-UI-backed registry component (`Combobox/ComboboxValue/Trigger/Clear/Input/Content/List/Item/Group/Label/Empty/Separator/Chips/Chip`), with search built into the trigger, multi-select chips, a clear button, and grouping | **Updated 2026-08-04**: `ShadcnCombobox<T>` single-select overload unchanged (default entry point); new multi-select overload (`values: List<T>`/`onValuesChange`) renders picks as removable `ShadcnBadge` chips; new `onClear: (() -> Unit)? = null` opt-in clear button; new `groupOf: ((T) -> String?)? = null` buckets a flat option list into headed groups (reuses `ShadcnCommandGroup`'s heading/separator rendering) | expanded/collapsed, filter-as-you-type, row select, multi-select chips, clear button, grouping; still **no `disabled`** | fixed `.width(200.dp)` on both trigger and panel, kept in sync only by matching literals — real anchors width to the trigger (`w-(--anchor-width)`) | Multi-select/clear/grouping fixed. Deliberate simplification kept: trigger stays a click-to-open button + separate dropdown panel (this library's existing `ShadcnSelect` pattern), not real's inline-input-by-default trigger — no established inline-combobox-input primitive exists here yet. `ShadcnCommand`'s KDoc inconsistency (falsely implying Combobox is built on it) also fixed |
...
| **Chart** | Type-agnostic wrapper: `ChartContainer`(injects per-series CSS color vars, wraps Recharts) + `ChartTooltip`/`ChartTooltipContent`(indicator styles, formatters) + `ChartLegend`; actual chart types delegated to Recharts children, not baked in | `ShadcnChartContainer`(aspect-ratio box only) + `ShadcnBarChart`/`LineChart` drawn directly via `Canvas` — architecturally inverted (chart type baked in vs. real's type-agnostic wrapper) | **Updated 2026-08-04**: ✅ hover (desktop)/tap (touch) tooltip added — a `pointerInput`+`awaitPointerEventScope` hit-test layer maps the pointer's x-position to the nearest data point using the same per-point pixel math the draw loop already uses, then renders a bordered `bg-popover` card with each series' label+value near the pointer, matching real `ChartTooltipContent`'s shape | aspect ratio 16:9 matches real's `aspect-video` default | Tooltip fixed. Still missing: light/dark per-series color switching (real's `ChartConfig.theme` has no analog — fixed single `Color` per series); no pie/area/radar variants |
...
2. ✅ *Fixed 2026-08-04* **`HoverCard` had zero open/close delay** — real's `openDelay=700ms`/`closeDelay=300ms` is the entire reason HoverCard exists as a component distinct from Tooltip; now matches. Content-hover-extends-close-timer still not covered (see §4 table).
3. ✅ *Fixed 2026-08-04* **`Dialog`/`AlertDialog` had no open/close animation at all**, while `Sheet`/`Drawer` (same overlay family) did — now Dialog/AlertDialog entrance-animate (`fadeIn` + `scaleIn`) too, consistent with the rest of the overlay family. None of the four animate on exit, a shared, documented `ShadcnModalOverlay` architectural limit, not a per-component gap anymore.
4. **Menu-family root cause, fixable once** — ✅ *partially fixed 2026-08-04*: `DropdownMenu`, `ContextMenu`, and `Menubar` all funnel through `ShadcnDropdownMenuScope`'s row composables, so a gap closed in `ShadcnDropdownMenu.kt` closes it everywhere that reuses it. `CheckboxItem`/`RadioGroup`/`RadioItem`/`Shortcut`/`inset` landed this way. Still open: `Sub`/`SubTrigger`/`SubContent` (submenu support) — deferred as its own, larger piece (nested overlay positioning + hover-open state machine, not a mechanical row addition like the rest of this list item).
...
6. ✅ *Fixed 2026-08-04* **`Combobox` matched a stale real recipe** — real shadcn/ui moved from "Popover + Command" to a first-class Base-UI-backed component (multi-select chips, clear button, grouping); `ShadcnCombobox` now has multi-select (chips), a clear button, and grouping too. Trigger stays click-to-open (not real's inline-input-by-default) as a documented simplification. `ShadcnCommand`'s KDoc, which falsely claimed `ShadcnCombobox` is built on it, is also fixed.
7. ✅ *Fixed 2026-08-04* **`Sheet` had no close button at all** (real shows one by default) — now reuses `ShadcnDialog`'s `DialogCloseButton`, with the same `showCloseButton`/`closeIcon` override params. Still open: **`Item` has zero interactive states** (no hover, no focus ring, despite real being fully focusable); **`Chart` has zero tooltip support** (real's largest sub-component).
