 ...
 *
 * Unlike [ShadcnBaseColor]/`ShadcnColors.kt`'s default palette (both migrated to named
 * `TwColors` shades -- see their doc comments), these hex literals are intentionally
 * **not** migrated: a scratch test comparing every value here against the full 26-hue/
 * 11-shade `TwColors` table found none are an exact or near-exact match to any named
 * shade (several are off by a wide margin, e.g. `Fuchsia`/`Lime`/`Sky`/`Rose`/`Green`).
 * That confirms this enum's own "corrected for ideal dark background contrast" comment
 * below -- these are deliberately hand-tuned away from stock Tailwind values, so
 * snapping them to the nearest named shade would silently change the actual accent
 * colors, not just rename them.
 */
@Immutable
