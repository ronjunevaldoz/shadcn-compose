...
- CI skill (`kmp-ci-github-actions`) for automated release
...
In `.github/workflows/release.yml` (from `kmp-ci-github-actions` skill):
...
- `kmp-feature-scaffold` — the KMP project structure being exported as an XCFramework
- `kmp-ci-github-actions` — CI job that assembles the XCFramework and publishes the binary target
- `kmp-expect-actual` — platform-specific iOS implementations bundled inside the XCFramework
