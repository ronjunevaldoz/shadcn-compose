...
Android 13+, `POST_NOTIFICATIONS` is a dangerous permission (see `kmp-permissions`
...
Use `Permission.PushNotifications` from the `kmp-permissions` skill:
...
- `kmp-permissions` — Android 13+ `POST_NOTIFICATIONS` permission is a
  dangerous permission; request it using the permissions skill's `PermissionController`
- `kmp-deep-linking` — notification taps route the user via `DeepLinkParser`;
  the same `DeepLink` sealed class and `toRoute()` extension are reused
- `kmp-workmanager` — token upload should be a one-time `WorkManager`
