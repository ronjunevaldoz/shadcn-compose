...
- You only need local debug logging — use `kmp-logging`
- You need user behaviour analytics — use `kmp-analytics`
...
- `kmp-logging` — structured logging; wire the logger wrapper into the breadcrumb bridge
- `kmp-analytics` — event tracking; crash reporting is about exceptions, not behaviour
- `kmp-expect-actual` — alternative approach if you prefer `expect class` over interface injection
- `kmp-dependency-injection` — Koin wiring for `CrashReporter` binding per platform
