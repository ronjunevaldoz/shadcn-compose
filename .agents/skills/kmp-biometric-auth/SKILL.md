...
- `kmp-mvi` — `BiometricResult` handling belongs in `onIntent`; the
  authenticate intent triggers the authenticator and updates `UiState`
- `kmp-dependency-injection` — `BiometricAuthenticator` requires a
  platform context (`FragmentActivity` on Android); bind it in the platform Koin module
- `kmp-permissions` — biometric authentication does not require a
