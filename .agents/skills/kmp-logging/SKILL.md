...
- `kmp-feature-scaffold` - logger wrapper lives in `:core:common`
- `kmp-unit-testing` - use a quiet test logger or no-op sink in `commonTest`
- `kmp-dependency-injection` - Koin wiring for the logger factory
- `kmp-crash-reporting` - breadcrumb bridge for crash reports
