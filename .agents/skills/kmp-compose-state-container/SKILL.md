---
name: kmp-compose-state-container
description: >
...
└── NO
    ...
    └── Is this state shared between sibling composables?
        ├── YES → hoist to parent (see kmp-compose-state-hoisting)
...
See `kmp-compose-state-hoisting` for full decision table and examples.
...
- `kmp-mvi` — ViewModel as the primary state container with `StateFlow` + `Channel<Effect>`
- `kmp-compose-state-hoisting` — when and where to hoist state vs keeping it in a container
- `kmp-presenter-module` — pure ViewModel pattern for containers with no Compose dependency
- `kmp-unit-testing` — testing ViewModel state transitions with Turbine
