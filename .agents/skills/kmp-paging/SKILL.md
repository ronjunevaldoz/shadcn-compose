...
- `kmp-network-layer` — `FooApi` interface and `safeRequest` wrapper that
  `FooPagingSource` calls; network errors map to `LoadResult.Error`
- `kmp-sqldelight-setup` — required for `RemoteMediator`; SQLDelight provides
  the local paging source via `Query.asPagingSourceFactory()`
- `kmp-mvi` — ViewModel still follows the MVI contract; `PagingData` flow
  lives alongside `StateFlow<UiState>`, not inside it
- `kmp-unit-testing` — `PagingSource.load()` is a suspend function testable
  with `runTest`; no special paging test library required for source-level tests
- `kmp-repository-pattern` — `FooRepository` interface exposes
