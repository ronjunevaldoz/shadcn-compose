---
name: kmp-skill-harvester
description: >-
  Reads docs/lessons/*.md files from one or more consumer KMP projects and
  proposes concrete amendments to the source skills in kmp-agent-skills.
...
**Freshness rule:** recheck `kmp-expert` and the source skill's
...
| GitHub issue (`lesson` label) | Consumer files a [Lesson Report](https://github.com/ronjunevaldoz/kmp-agent-skills/issues/new?template=lesson_report.yml) issue on this repo | Any external user — no repo access needed |
...
gh issue view <number> --repo ronjunevaldoz/kmp-agent-skills
```
...
| `gap` (high severity, new domain) | Propose a new skill — name it `kmp-<topic>` |
...
```
## Proposed new skill: kmp-<topic>

Reason: <N> lessons report a gap that no existing skill covers.
Suggested name: kmp-<topic>
Covers: <one-sentence scope>
Run: /kmp-new-skill kmp-<topic>
```
The suggested name must start with `kmp-`. Never suggest a bare
...
See `kmp-offline-first` for the connectivity monitor pattern.
```
...
- For `create_skill` proposals: the suggested skill name must start with `kmp-`
- Confirm the report excludes lessons already marked `applied: true`

Run `python3 skills/kmp-audit/scripts/audit_skills_repo.py .` to catch naming and line-limit violations in harvested output files.
...
- `kmp-lessons` — consumer-side skill for writing the lesson
  files this harvester reads
- `kmp-audit` — run after harvesting to verify the amendments
  were applied correctly and no new gaps were introduced
- `kmp-project-docs-maintainer` — owns PLAN.md and CHANGELOG
