...
- `kmp-feature-scaffold` — `build-logic` convention plugins where `BuildKonfig` is applied
- `kmp-ci-github-actions` — CI secret injection via `env:` that feeds BuildKonfig values
- `kmp-network-layer` — consumes `BuildKonfig.BASE_URL` and `BuildKonfig.DEBUG`
- `kmp-logging` — consumes `BuildKonfig.DEBUG` to enable verbose log writers
