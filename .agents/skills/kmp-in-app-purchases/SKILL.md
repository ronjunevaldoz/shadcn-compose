...
- `kmp-expect-actual` — the `BillingRepository` implementation uses expect/actual per platform
- `kmp-mvi` — `PremiumViewModel` follows the full MVI contract
- `kmp-dependency-injection` — `BillingRepository` is a singleton bound per platform in Koin
- `kmp-network-layer` — required for server-side receipt validation
