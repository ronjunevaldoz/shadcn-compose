...
- `kmp-ktor-auth-service` — auth service depends on the `UserRepository` built here
- `kmp-kotlin-rpc` — RPC service implementations delegate to MongoDB repositories
- `kmp-repository-pattern` — the same repository interface/implementation pattern applied server-side
- `kmp-dependency-injection` — `MongoClient` and collection bindings are registered as Koin singletons
