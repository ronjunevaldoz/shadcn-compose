...
- `kmp-mvi` — `PermissionState` lives inside `UiState` as a field;
  permission request is an `Intent`
- `kmp-dependency-injection` — `PermissionController` is a Koin single
  bound in platform modules because it requires a platform context or activity
- `kmp-push-notifications` — push notification permissions on Android 13+
