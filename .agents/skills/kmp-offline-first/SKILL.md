...
the one in the `kmp-sqldelight-setup` skill.
...
- `kmp-sqldelight-setup` — local database; required for single-source-of-truth reads
- `kmp-repository-pattern` — the Repository interface that SyncManager plugs into
- `kmp-workmanager` — platform-specific background scheduling for Android/iOS sync
- `kmp-network-layer` — network calls the SyncWorker makes to the server
- `kmp-mvi` — SyncState surfaces through UiState; MVI pattern keeps it clean
