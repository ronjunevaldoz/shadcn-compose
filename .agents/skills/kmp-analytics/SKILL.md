...
- `kmp-mvi` — analytics calls go in `onIntent` after state updates, not in composables
- `kmp-dependency-injection` — platform `analyticsModule` wired in platform-specific Koin setup
- `kmp-unit-testing` — `FakeAnalytics` is the canonical fake; test event names and params, not SDK calls
