...
`ronjunevaldoz/kmp-agent-skills` repo directly from your current session.
...
`kmp-lessons` drafts an issue using this command's Step 4 template
...
| `kmp-new-skill` | `enhancement` | A KMP / JNI / Android concern is not covered by any existing skill |
...
```bash
gh issue list --repo ronjunevaldoz/kmp-agent-skills --state open --search "$ARGUMENTS"
```

If a duplicate exists, show the title and URL and ask:
**Add a comment to the existing issue / Open a new issue / Cancel**

On "Add a comment": run `gh issue comment <number> --repo ronjunevaldoz/kmp-agent-skills --body "<body>"` and stop.
...
| Field | Question |
|---|---|
| **Skill name** | Which skill triggered this? (e.g. `kmp-jni-pro`) |
...
- Never file an issue if `$ARGUMENTS` describes something already in CHANGELOG.md — check first:
  `gh api repos/ronjunevaldoz/kmp-agent-skills/contents/CHANGELOG.md --jq '.content' | base64 -d | head -60`
