...
- `kmp-navigation` — `NavHost` setup and `NavController` wiring;
  deep links are registered as `navDeepLink` blocks inside `composable { }` builders
- `kmp-mvi` — deep-link handling triggers a `NavigateTo` side effect,
  not a state mutation; the ViewModel emits a `Channel`-based `Effect`
- `kmp-unit-testing` — `DeepLinkParser` is a pure object with no
