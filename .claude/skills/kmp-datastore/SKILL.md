...
- `kmp-feature-scaffold` — `:core:datastore` follows the same convention plugin pattern
- `kmp-dependency-injection` — Koin platform-specific modules for Android vs iOS DataStore factory
- `kmp-presenter-module` — ViewModels consume `PreferencesDataSource` via constructor injection
- `kmp-unit-testing` — fake `PreferencesDataSource` for ViewModel tests; no DataStore needed in tests
- `kmp-expect-actual` — `createDataStore {}` is a canonical expect/actual use case
