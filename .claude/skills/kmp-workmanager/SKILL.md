...
- `kmp-push-notifications` — push token upload after `onNewToken` is the
  primary use case; enqueue `WorkTag.UploadToken` from `FirebaseMessagingService`
- `kmp-dependency-injection` — `BackgroundScheduler` is a Koin single
  in the platform module; `AppWorker` accesses use cases via `koin-android-workmanager`
- `kmp-unit-testing` — use case logic called by the worker is tested
