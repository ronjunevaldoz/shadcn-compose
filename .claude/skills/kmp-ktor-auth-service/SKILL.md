...
- `kmp-kotlin-rpc` — auth guards the Ktor RPC route using bearer/JWT from this skill
- `kmp-mongodb-database` — `UserRepository` used by `AuthService` for credential lookup
- `kmp-network-layer` — client-side token injection is handled by the Ktor auth plugin
- `kmp-feature-scaffold` — server modules live alongside the shared and client modules from this skill
